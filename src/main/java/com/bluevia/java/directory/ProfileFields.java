/**
 * 
 * @category bluevia
 * @package com.bluevia.java.directory.client
 * @copyright Copyright (c) 2010 Telefonica I+D (http://www.tid.es)
 * @author Bluevia <support@bluevia.com>
 * 
 * 
 *          BlueVia is a global iniciative of Telefonica delivered by Movistar
 *          and O2. Please, check out http://www.bluevia.com and if you need
 *          more information contact us at support@bluevia.com
 */

package com.bluevia.java.directory;

/**
 * ProfileFields
 * 
 * @package com.bluevia.java.directory.client
 */

/**
 * <p>Java class for defined Profile Fields to filter out information in the request
 * 
 */

public enum ProfileFields {
    userType, imsi, icb, ocb, language, simType, parentalControl, creditControl, 
    diversionMsisdn, enterpriseName, roaming, operatorId, mmsStatus, segment, 
    subsegment, subscribedService, lastUpdated
}
