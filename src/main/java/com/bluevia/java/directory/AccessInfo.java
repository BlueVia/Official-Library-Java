...
/**
 * <p>Java class for get UserAccessInfo from Directory API
 * 
 */

public class AccessInfo extends DirectoryClient {
     ...
     */
    public AccessInfo(OAuthToken consumerToken, OAuthToken accessToken, Mode mode) throws JAXBException {
        super(consumerToken, accessToken, mode);
    }

    /**
     * Get UserAccessInfo from phoneNumber
     * 
     * @param alias_accessToken
     * @param dataSets {@link AccessInfoFields}
     * @return
     * @throws JAXBException
     * @throws BlueviaException
     */
    public UserAccessInfoType get(AccessInfoFields[] dataSets) throws JAXBException, BlueviaException {

        StringBuffer sb = new StringBuffer();
        if (dataSets != null) {
            for (AccessInfoFields e : dataSets) {
                sb.append(e.toString()).append(",");
            }
            sb.deleteCharAt(sb.length() - 1);
        }
        String url = this.uri + "alias:" + this.restConnector.getAccessToken() + "/UserInfo/UserAccessInfo?version=v1";
        if (dataSets != null) {
            try {
        ...
        JAXBElement<UserAccessInfoType> e = this.u.unmarshal(new StreamSource(new StringReader(res)), UserAccessInfoType.class);
        return e.getValue();
    }
